...
// Generar array de clientes basándose en los archivos disponibles
const generarClientes = () => {
  return Object.entries(logoModules).map(([path, module]) => {
    const fileName = path.split("/").pop();
    const nombreBase = fileName.split(".")[0]; // quitar extensión
    
    // Generar nombre legible: "logo berti.png" -> "Berti"
    const nombre = nombreBase
      .replace(/^logo\s*/i, "") // quitar "logo" del inicio
